        ...
        dispatch(
            callApi({
                operationId: UrlBuilder.ntrcaApi(
                    `designation/details/${props.match.params.id}`
                ),
                output: "details",
                storeName: "PostDetails",
            })
        );
        ...
        headerSlot: () => (
            <>
                <Link to="/portal/settings/post/list">
                    ...
                    >
                        <FontAwesomeIcon icon={faList} className="me-2" /> পদের
                        তালিকা দেখুন
                    </Button>
                </Link>
            </>
        ...
        <DefaultCard className="mb-50" {...cardProps}>
            <Card border="white" className="table-wrapper table-responsive">
                <Card.Body>
                    {loading && <ProgressBar />}
                    <Formik
                        initialValues={Post.fromJson(details.data)}
                    ...
                    >
                        {(props) => {
                            return <TeacherPostForm {...props} from="edit" />;
                        }}
                    </Formik>
                </Card.Body>
            </Card>
...
export default TeacherpostEdit;
